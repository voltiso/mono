...
namespace V {
template <Size NUM_ITEMS>
class String : public string::Custom<
                 Options<option::Extents<ValuePack<NUM_ITEMS>>, option::Final<String<NUM_ITEMS>>>> {
...
public:
	...
	consteval String(const char (&items)[NUM_ITEMS_WITH_NULL]) : Base(tag::COPY_CONSTEVAL, items) {
...
public:
	...
	template <class... Args>
	[[nodiscard]] static VOLTISO_FORCE_INLINE constexpr auto from(Args &&...args) {
...
public:
	...
	template <class Other>
	  requires(get::EXTENT<Other> != extent::DYNAMIC && get::EXTENT<Other> != extent::UNBOUND)
	constexpr auto
	operator<<(this auto &&self, Other &&other) { // `other` is the RHS of String << Other
		...
		return self.template as<Base>().operator<<(
		  ConstStringView(std::forward<Other>(other)) // Forward 'other' to ConstStringView
		);
	}
}; // class String
// deduction guide
...
template <class... Args>
[[nodiscard]] static VOLTISO_FORCE_INLINE constexpr auto
from(Args &&...args) noexcept(noexcept(tensor::from(std::forward<Args>(args)...))) {
