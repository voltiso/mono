...
template <is::Options O> class Arithmetic {
	...
	using _ResultBox = std::conditional_t<
	  std::is_base_of_v<Arithmetic, std::remove_cvref_t<Lhs>>, std::remove_cvref_t<Lhs>,
...
public:
	constexpr auto operator+(this auto &&self) noexcept(noexcept(+self.value))
	  ...
	  requires _Config::arithmetic && requires { +self.value; }
	{
		_check();
	...
	constexpr auto operator++(this auto &self, int) noexcept(noexcept(self.value++))
	  -> std::remove_cvref_t<decltype(self)>
	  requires _Config::arithmetic && requires { self.value++; }
	{
		_check();
...
#define V_BOX_MIXIN_ARITHMETIC_OP(OP_ASSIGN, OP)                                                   \
	...
	constexpr auto operator OP_ASSIGN(this auto &self, Other &&other) noexcept(                      \
	  ...
	  ->decltype(self)                                                                               \
	  requires _Config::arithmetic &&                                                                \
	           (!std::is_base_of_v<Arithmetic, std::remove_cvref_t<Other>> ||                        \
	...
	friend constexpr auto operator OP(Lhs &&lhs, Rhs &&rhs)                                          \
	  ->decltype(auto)                                                                               \
	  requires _Config::arithmetic &&                                                                \
	           (std::is_base_of_v<Arithmetic, std::remove_cvref_t<Lhs>> ||                           \
	            std::is_base_of_v<Arithmetic, std::remove_cvref_t<Rhs>>) &&                          \
	...
	V_BOX_MIXIN_ARITHMETIC_OP(+=, +)
	V_BOX_MIXIN_ARITHMETIC_OP(-=, -)
	V_BOX_MIXIN_ARITHMETIC_OP(*=, *)
	V_BOX_MIXIN_ARITHMETIC_OP(/=, /)
	V_BOX_MIXIN_ARITHMETIC_OP(%=, %)

#undef V_BOX_MIXIN_ARITHMETIC_OP
};
