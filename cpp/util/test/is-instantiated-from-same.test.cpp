...
TEST_F(IsInstantiatedFromSameTest, TypeOnly_SameTemplateDifferentArgs) {
	...
	static_assert(
	  is::InstantiatedFromSame<std::vector<int>, std::vector<double>>,
	  "Vectors with different types");
	static_assert(
	  is::InstantiatedFromSame<std::list<int>, std::list<float>>,
	  "Lists with different types");
	static_assert(
	  is::InstantiatedFromSame<std::pair<int, double>, std::pair<char, MyClass>>,
	...
	static_assert(
	  is::InstantiatedFromSame<MyTemplate1<int>, MyTemplate1<float>>,
	  "MyTemplate1 with different types");
	static_assert(
	  is::InstantiatedFromSame<MyTemplate1<char, double>, MyTemplate1<int>>,
	  "MyTemplate1 with different arg counts");

	ASSERT_TRUE(
	  (is::InstantiatedFromSame<std::vector<int>, std::vector<double>>));
	ASSERT_TRUE(
	  (is::InstantiatedFromSame<MyTemplate1<int>, MyTemplate1<float, double>>));
}

TEST_F(IsInstantiatedFromSameTest, TypeOnly_SameTemplateSameArgs) {
	static_assert(
	  is::InstantiatedFromSame<std::vector<int>, std::vector<int>>,
	  "Vector with same type");
	static_assert(
	  is::InstantiatedFromSame<MyTemplate1<MyStruct>, MyTemplate1<MyStruct>>,
	  "MyTemplate1 with same type");
	ASSERT_TRUE(
	  (is::InstantiatedFromSame<std::list<std::string>, std::list<std::string>>));
}
