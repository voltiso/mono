...
#include "v/get/numItems"
#include "v/get/numSlots"
#include "v/is_trivially_relocatable"
#include "v/option/Allocator"
#include "v/option/IN_PLACE"
#include "v/option/IN_PLACE_ONLY"
...
public:
	[[nodiscard]] VOLTISO_FORCE_INLINE constexpr auto numItems() const {
...
public:
	...
	Custom(Custom &&other) noexcept
		...
		if constexpr (NUM_IN_PLACE_SLOTS == 0) {
			...
			static_assert(is_trivially_relocatable<Storage<Item>>);
			memcpy(
			  &this->inPlaceItems[0], &other.inPlaceItems[0],
			  sizeof(Item) * other._numItems);
		} else {
...
public:
	...
	template <class Items>
	  requires(
	    std::is_reference_v<Items> ||
	    std::is_const_v<std::remove_reference_t<Items>>)
	constexpr Custom(tag::ExplicitCopy, Items &&items) {
...
template <class Options>
constexpr auto is_trivially_relocatable<dynamicArray::Custom<Options>> = true;
