...
TEST(OptionsWith, ReplaceExistingCustomTemplateOption) {
	...
	// --- Diagnostic Assert ---
	// Check if is_InstantiatedFromSame correctly identifies these as the same
	// kind. This is expected to be TRUE for the replacement logic to work. If
	// this fails, the is_InstantiatedFromSame trait needs a specialization for
	...
	static_assert(
	  ...
	  "DIAGNOSTIC: is_InstantiatedFromSame is not identifying two "
...
TEST(OptionsWith, ReplaceExistingTypeOptionKind) { // Renamed for clarity
	...
	// Add Item<double>. is_InstantiatedFromSame(Item<double>, Item<int>) is
