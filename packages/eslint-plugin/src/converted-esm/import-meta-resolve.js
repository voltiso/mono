...
function find(dir) {
	try {
		const string = import_fs.default.readFileSync(
			import_path.default.toNamespacedPath(
				import_path.default.join(dir, 'package.json'),
			),
			'utf8',
		)
		return { string }
...
codes.ERR_INVALID_PACKAGE_CONFIG = createError(
	...
	(path4, base, message) => {
		return `Invalid package config ${path4}${
	...
	if (parsed.protocol === 'file:') {
		...
		if (!format2) {
			throw new ERR_UNKNOWN_FILE_EXTENSION(
				ext,
				(0, import_url.fileURLToPath)(url),
			)
		}
...
function emitFolderMapDeprecation(match, pjsonUrl, isExports, base) {
	...
	process.emitWarning(
		`Use of deprecated folder mapping "${match}" in the ${
			...
			base ? ` imported from ${(0, import_url2.fileURLToPath)(base)}` : ''
		}.
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
	...
	const pkgPath = (0, import_url2.fileURLToPath)(
		new import_url2.URL('.', packageJsonUrl),
	)
...
function getPackageConfig(path4, specifier, base) {
	...
	try {
		...
		throw new ERR_INVALID_PACKAGE_CONFIG(
			path4,
			(base ? `"${specifier}" from ` : '') +
				(0, import_url2.fileURLToPath)(base || specifier),
			error.message,
		)
	}
...
function getPackageScopeConfig(resolved) {
	...
	while (true) {
		...
		const packageConfig2 = getPackageConfig(
			(0, import_url2.fileURLToPath)(packageJsonUrl),
			resolved,
		)
...
function throwInvalidSubpath(subpath, packageJsonUrl, internal, base) {
	...
	}" resolution of ${(0, import_url2.fileURLToPath)(packageJsonUrl)}`
	throw new ERR_INVALID_MODULE_SPECIFIER(
		subpath,
		reason,
		base && (0, import_url2.fileURLToPath)(base),
...
function packageResolve(specifier, base, conditions) {
	...
	const packageConfig = getPackageScopeConfig(base)
	if (packageConfig.exists) {
		const packageJsonUrl2 = (0, import_url2.pathToFileURL)(
			packageConfig.pjsonPath,
		)
		if (
	...
	let packageJsonUrl = new import_url2.URL(
	...
	do {
		...
		if (!stat.isDirectory()) {
			...
			packageJsonUrl = new import_url2.URL(
...
/** @type {import('import-meta-resolve/lib/resolve').defaultResolve} */
function defaultResolve(specifier, context = {}) {
	...
	const old = url
	url = (0, import_url2.pathToFileURL)(
		real + (urlPath.endsWith(import_path3.default.sep) ? '/' : ''),
	)
