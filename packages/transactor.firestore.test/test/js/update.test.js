...
describe('update', function () {
	it('should update nested field', async function () {
		...
		await expect(db('person', 'artur').dataWithoutId()).resolves.toMatchObject(expected)
		await expect(db('person', 'artur').dataWithId()).resolves.toMatchObject({ ...expected, id })
	...
	it('should create objects if needed', async function () {
		...
		await expect(db('a/a').dataWithId()).resolves.toMatchObject({ id: 'a', x: 1, y: { yy: 2 } })
		// @ts-ignore
