...
const db = createTransactor(firestore, { requireSchemas: false, refCounters: false })
...
describe('incrementIt', function () {
	...
	it('passes-through updates to firestore (no throw on undefined += x)', async function () {
		...
		await expect(db('friend/artur/project/super').update({ numProjects: incrementIt(1) })).rejects.toThrow('NOT_FOUND') // no such document
		await db('friend/artur/project/tds').update({ numProjects: incrementIt(1) }) // this work unfortunately - FireStore allows this
		await expect(db('friend/artur/project/tds').dataWithId()).resolves.toStrictEqual({ id: 'tds', numProjects: 1 })
	})
	...
	it('passes-through updates to firestore (no throw on undefined += x) - in transaction', async function () {
		...
		await db('friend/artur/project/tds').set({})
		await db.runTransaction(async db => {
			await db('friend/artur/project/tds').update({ numProjects: incrementIt(1) })
		})
		await expect(db('friend/artur/project/tds').data).resolves.toStrictEqual({ numProjects: 1 })
	})
})
