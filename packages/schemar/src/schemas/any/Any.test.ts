...
describe('any', () => {
	...
	it('simple', () => {
		...
		type A1 = GetOutputType<typeof s.any>
		type A2 = GetInputType<typeof s.any>
		...
		expect(() => s.any.extends(s.never)).toThrow('asd')
		expect(() => s.any.extends(s.number)).toThrow('asd')
		...
		expect(() => s.any.extends(s.undefined)).toThrow('asd')
