...
import { GetInputType, GetOutputType, GetType_ } from '../../GetType'
import { NumberOptions } from './_/NumberOptions'
import { CustomNumber } from './CustomNumber'
import * as s from '..'

describe('s.number', () => {
	it('generic', <O extends NumberOptions>() => {
		expect.assertions(0)

		Assert.is<s.INumber<O>, s.INumber>()

		// eslint-disable-next-line @typescript-eslint/ban-ts-comment
		// @ts-ignore error TS2589: Type instantiation is excessively deep and possibly infinite.
		Assert.is<CustomNumber<O>, s.INumber<O>>()
	...
	it('default', () => {
		...
		expect(s.number.optional.tryValidate(undefined).value).toBeUndefined()
		expect(s.number.optional.tryValidate(undefined).isValid).toBeFalsy()

		const n = s.number.default(123)
		expect(n.tryValidate(undefined).value).toBe(123)
